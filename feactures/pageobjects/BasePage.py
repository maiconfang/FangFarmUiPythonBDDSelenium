...
class BasePage:
    ...
    def value(self, locator):
        log.logger.info("Getting the value of the element: " + str(locator))
        if str(locator).endswith("_XPATH"):
            return self.driver.find_element(By.XPATH, configReader.readConfig("locators", locator)).text
        elif str(locator).endswith("_CSS"):
            return self.driver.find_element(By.CSS_SELECTOR, configReader.readConfig("locators", locator)).text
        elif str(locator).endswith("_ID"):
